...
async def _bootstrap_telemetry(self):
    ...
    try:
        ...
        if client_cert and ca_cert and os.path.exists(client_cert) and os.path.exists(ca_cert):
            ...
            ssl_ctx.check_hostname = False  # Self-signed
            ssl_ctx.verify_mode = ssl.CERT_REQUIRED
            logger.info(f"mTLS enabled: {client_cert}")

        # WebSocket URL - mit oder ohne separaten Port
        if wss_port and ssl_ctx:
            # Neuer mTLS-Port (44433)
            ws_base = self.server_url.replace("https://", "").replace("http://", "")
            ws_url = f"wss://{ws_base}:{wss_port}/mcp?session_id={self.session_id}&tier={self.tier}&mode=telemetry"
        else:
            # Fallback: Standard-Route mit Token
            ...
            while True:
                try:
                    ...
                    async with aiohttp.ClientSession(connector=connector) as session:
                        async with session.ws_connect(ws_url, heartbeat=30, ssl=ssl_ctx) as ws:
                            ...
                            await ws.send_json({
                                ...
                                "params": {
                                    ...
                                    "auth": "mtls" if ssl_ctx else "token"
                                }
                            })
                            ...
                            async for msg in ws:
                                if msg.type == aiohttp.WSMsgType.TEXT:
                                    ...
                                    if method == "ping":
                                        ...
                                        logger.warning(f"⚠️ Server wollte Tool aufrufen - IGNORIERT")
                                        request_id = data.get("id")
                                        if request_id:
                                            await ws.send_json({
                                                "jsonrpc": "2.0",
                                                "id": request_id,
                                                "error": {"code": -32600, "message": "Remote execution disabled"}
                                            })
        ...
        logger.info(f"🚀 Telemetrie gestartet ({'mTLS' if ssl_ctx else 'Token'})")
